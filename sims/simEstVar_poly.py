  ...
  for j in range(numTrials):
    ...
    for k in range(1, (numTpts-j)):
      censQ[j][k-1]  = np.asarray(d[['one', # list of design matrices 
            'j_{}'.format(j) + '_{}'.format(k),
            'jSqr_{}'.format(j) + '_{}'.format(k),
            'L1_c', 'L2', 'L3']])
      censY[j][k-1]  = np.asarray(d[["oneMinusC_{}".format(j) + "_{}".format(k)]])
      atRisk[j][k-1] = np.asarray(d[["Y_{}".format(j) + "_{}".format(k)]])
      
      ### hazard model design matrices
      if analysis=="naive":
        ocX[j][k-1]  = np.asarray(d[['one', 
          'kPlusTrial_{}'.format(j)      + '_{}'.format(k),
          'kPlusTrialSqr_{}'.format(j)   + '_{}'.format(k),
          'Z_{}'.format(j), 
          'Z_k_{}'.format(j)    + '_{}'.format(k),
          'Z_kSqr_{}'.format(j) + '_{}'.format(k)]])
        
        surv1mat[j][k-1] = np.asarray(d[['one', 
          'kPlusTrial_{}'.format(j)      + '_{}'.format(k), 
          'kPlusTrialSqr_{}'.format(j)   + '_{}'.format(k),
          'one', 
          'k_{}'.format(j)      + '_{}'.format(k), 
          'kSqr_{}'.format(j)   + '_{}'.format(k)]])
        ...
        ocX[j][k-1] = np.asarray(d[['one', 
          ...
          'kPlusTrialSqr_{}'.format(j) + '_{}'.format(k),
          'Z_{}'.format(j), 
          'Z_kPlusTrial_{}'.format(j)    + '_{}'.format(k),
          'Z_kPlusTrialSqr_{}'.format(j) + '_{}'.format(k),
          'Z_k_{}'.format(j)    + '_{}'.format(k),
          'Z_kSqr_{}'.format(j) + '_{}'.format(k)]])
